...
39. **Preflight ordering is mechanically binding.** For every agent/chair covered by Rule 36, steps 36(a)–(h) are prerequisites, not execution work. The complete preflight must be issued and durably recorded **before** the first substantive execution, mutation, model dispatch, canary, test, or endpoint verification. Read-only retrieval needed solely to satisfy the preflight prerequisites is permitted before the preflight; substantive task execution is not. If substantive execution occurs first, the run is an immediate process `FAIL`: stop, record the violation/allowance waste, apply Rule 38, then start a fresh run from retrieval. A later preflight cannot retroactively validate earlier execution.

40. **Universal enforcement scope.** Rules 36–39 apply identically to Luna, Sol, Gemini Prime, Grace, DeepSeek, Claude, Codex, every `(or)` shadow, and every future HumanVibe executor/chair. No agent may self-exempt, downgrade the gate, or treat the preflight as display-only text.